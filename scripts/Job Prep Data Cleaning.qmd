...
```{r}
# Reverse-score items on a 6-point Likert scale
reverse_items <- c("item_4", "item_9", "item_12", "item_13")

survey <- survey %>%
  mutate(across(all_of(reverse_items), ~ 7 - ., .names = "rev_{.col}"))

# Replace originals with reversed scores
survey_tidy <- survey %>%
  mutate(item_4 = rev_item_4,
         item_9 = rev_item_9,
         item_12 = rev_item_12,
         item_13 = rev_item_13) %>%
  select(-starts_with("rev_"))

# Compute total optimism score
survey_tidy <- survey_tidy %>%
  rowwise() %>%
  mutate(optimism_score = sum(c_across(starts_with("item_")), na.rm = TRUE)) %>%
  ungroup()
```
